...
class notification {
    ...
    protected function insert_schedule($data, $newschedule=false, $newfrequency=false) {
        ...
        if ((!$newschedule && !$newfrequency) && $DB->record_exists('pulseaction_notification_sch', [
    ...
    public function create_schedule_foruser($userid, $lastrun='', $notifycount=0, $expectedruntime=null,
        $isnewuser=false, $newschedule=false, $newfrequency=false) {
        ...
        if (!$this->verfiy_instance_contains_condition()
            || !instances::create($this->notificationdata->instanceid)->find_user_completion_conditions(
                $this->instancedata->condition, $this->instancedata, $userid, $isnewuser)
            || ($newschedule
                && notify_users::is_suppress_reached($this->notificationdata, $userid, $this->instancedata->course, null))) {
            // Remove the user condition.
            $this->disable_user_schedule($userid);
            return true;
        }

        [$findnotifycount, $frequencycount] = $this->find_notify_count($userid);
        ...
        if (!$newfrequency || ($this->instancedata->frequencylimit >= 1 && !$this->frequncy_available($userid, $frequencycount))) {
