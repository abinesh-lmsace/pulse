 ...
 */
class backup_pulseaddon_reaction_subplugin extends backup_subplugin {
    ...
    protected function define_pulse_subplugin_structure() {
        ...
        $subpluginelement = new backup_nested_element('pulseaddon_reaction', ['id'], [
        ...
        if ($userinfo) {
            $subpluginelement->set_source_table('pulseaddon_reaction_tokens', ['pulseid' => backup::VAR_PARENTID]);
            $subpluginelement->annotate_ids('user', 'userid');
            $subpluginelement->annotate_ids('user', 'relateduserid');
        }
