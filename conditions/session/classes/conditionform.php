...
class conditionform extends \mod_pulse\automation\condition_base {
    ...
    public function load_template_form(&$mform, $forminstance) {
        ...
        $mform->addElement('static', 'condition[session][modules]', get_string('sessionmodule', 'pulsecondition_session'),
            get_string('sessionmodule_help', 'pulsecondition_session'));
    ...
    public static function get_session_notifications($id) {
        ...
        $sql = "SELECT *, ai.id as id, ai.id as instanceid
                ...
                WHERE $like AND (co.status > 0 OR (co.status IS NULL AND ai.templateid IN (
                        SELECT c.templateid FROM {pulse_condition} c WHERE c.triggercondition = 'session'
                    )
                ))";
        // Params.
