...
class preset_customize_restore extends restore_structure_step {
    ...
    public function define_structure() {
        ...
        if ($this->get_name() == 'pulseaddon') {
            $paths[] = new restore_path_element('pulseaddon_reminder',
                '/activity/pulse/subplugin_pulseaddon_reminder_pulse/pulseaddon_reminder');
        }
    ...
    /**
     * Process the puslepro element restore.
     * The data is set as class level using this we can able to fetch the pulse addon reminder data from XML.
     *
     * @param array $data
     * @return void
     */
    public function process_pulseaddon_reminder($data) {
        $this->data += $data;
    }
}
